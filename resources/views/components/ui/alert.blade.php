...
<div {{ $attributes->merge(['class' => "rounded-lg border p-4 {$typeConfig['bg']} {$typeConfig['border']}"]) }}
     x-data="{ show: true }"
     x-show="show"
     x-transition>
    <div class="flex">
        ...
        <div class="ml-3 flex-1">
            ...
            @endif
            <div class="text-sm {{ $typeConfig['text'] }} @if($title) mt-1 @endif">
                {{ $slot }}
            </div>
        </div>
        ...
        <div class="ml-auto pl-3">
            <button @click="show = false" 
                    class="-mx-1.5 -my-1.5 rounded-lg p-1.5 inline-flex {{ $typeConfig['text'] }} hover:bg-white/50 dark:hover:bg-black/20">
